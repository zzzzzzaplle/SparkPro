...
object TopProvince {
  def main(): Unit = {
    ...
    filteredCityDF.createOrReplaceTempView("province")

    val topCitiesDF = dfWithSaleQtty.join(filteredCityDF, dfWithSaleQtty("user_site_province_id") === filteredCityDF("dim_province_id"))
    ...
    topCitiesDF.write.jdbc("jdbc:mysql://192.168.56.104:3306/sparkproject?useSSL=false","TopProvince",prop)
