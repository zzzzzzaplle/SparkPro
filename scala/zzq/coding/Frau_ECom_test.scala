...
object Frau_ECom_test {
  def main(): Unit = {
    ...
    val dataPath = "hdfs://niit-master:9000/user/niit/src/main/scala/zzq/input/dataset.csv"
    val df = spark.read
   ...
   val fraudHourTransactions = hashedData.filter($"Is_Fraudulent" === 1)
       .withColumn("Time_Slot", when($"trans_hour" >= 23 || $"trans_hour" <= 5, "23-5")
         .when($"trans_hour" >= 6 && $"trans_hour" <= 10, "6-10")
         .when($"trans_hour" >= 11 && $"trans_hour" <= 17, "11-17")
         .otherwise("18-22"))
